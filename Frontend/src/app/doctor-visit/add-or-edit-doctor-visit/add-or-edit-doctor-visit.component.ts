    ...
    if (this.action == 'Add') {
      this.doctorVisitService.addDoctorVisit(this.doctorVisit).subscribe(
        (doctorVisit) => { this.location.back(); 
          this._snackBar.open('Doctor visit added successfully', 'x', { duration: 3000 })},
        (err) => this._snackBar.open('Error while adding Doctor visit', 'x', { duration: 3000 })
      )
    } else {
      this.doctorVisitService.editDoctorVisit(this.doctorVisit).subscribe(
        (doctorVisit) => { this.location.back(); 
          this._snackBar.open('Doctor visit updated successfully', 'x', { duration: 3000 })},
        (err) => this._snackBar.open('Error while updating Doctor visit', 'x', { duration: 3000 })
      )
    }
  }
