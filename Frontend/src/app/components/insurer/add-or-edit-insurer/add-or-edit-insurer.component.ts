    ...
    if(this.action == 'add'){
        this.insurerService.addInsurer(this.insurer).subscribe(
          (data)=>{
            ...
            this._snackBar.open('Insurer added successfully', 'x', { duration: 3000, panelClass: ['mat-success'] });
          }    
          },
          err=>this._snackBar.open('Error while adding Insurer', 'x', { duration: 3000, panelClass: ['snackbar-error']  })
        )
    }else{
      this.insurerService.editInsurer(insurerid,this.insurer).subscribe(
        (data)=>{if(data['success']){ this.router.navigate(['/home/insurers']);
        this._snackBar.open('Insurer updated successfully', 'x', { duration: 3000, panelClass: ['snackbar-success'] });
      }},
        err=>this._snackBar.open('Error while updating Insurer', 'x', { duration: 3000, panelClass: ['snackbar-error']  })
      )
    }
  }
