...
export class InsurerListComponent implements OnInit {
  insurer: any;
  deleteInsurerId: any;
  displayedColumns: string[] = ['name', 'location', 'pointOfContact', 'phone', 'actions'];
  ...
  fetchInsurers() {
    this.insurerService.fetchInsurers().subscribe(
      ...
      (err) => this._snackBar.open('Error while fetching Insurers', 'x', { duration: 3000, panelClass: ['snackbar-error'] })
    );
  }
    ...
    this.insurerService.deleteInsurers(insurerId).subscribe(
      (success) => {
        if (success['success']) {
          ...
          this._snackBar.open('Insurer deleted successfully', 'x', { duration: 3000, panelClass: ['snackbar-success'] });
        }
      },
      (err) => {
        this._snackBar.open('Error while deleting Insurer', 'x', { duration: 3000, panelClass: ['snackbar-error'] })
      }
    )
  }
