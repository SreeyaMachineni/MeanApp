...
export class PackagesComponent implements OnInit {
  ...
  deletePackageId: any;
  insurer:Insurer;
  displayedColumns: string[] = ['name', 'insCategory', 'networkHospitals', 'premiumAmnt','maxSumAssured', 'diseasesCovered', 'requiredDocs', 'actions'];
  ...
  fetchPackages(insurerId){
    this.packageService.fetchPackages(insurerId).subscribe(
      ...
      (err)=>this._snackBar.open('Error while fetching Packages', 'x', { duration: 3000, panelClass: ['snackbar-error'] })
