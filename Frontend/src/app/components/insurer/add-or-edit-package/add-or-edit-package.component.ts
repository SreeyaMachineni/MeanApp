  ...
  saveit(packageId) {
    ...
    if (this.action == 'add') {
      this.packageService.addPackage(this.package).subscribe(
        ...
        err => this._snackBar.open('Error while adding Package', 'x', { duration: 3000, panelClass: ['snackbar-error'] })
      )
    }
    else {
      this.packageService.editPackage(packageId, this.package, true).subscribe(
        ...
        err => this._snackBar.open('Error while updating Package POCs', 'x', { duration: 3000, panelClass: ['snackbar-error'] })
      )
    }
  }
