...
router.get('/deletePackage/:packageId', (req, res) => {
    Package.deletePackage(req.params.packageId, (err, success) => {
        if (err) throw err;
            ...
            res.json({ success: true, msg: 'Deleted pack' });
        }
    })
})
...
router.post('/editPackage/:packageId', (req, res) => {
    Package.findById(req.params.packageId, (err, package) => {
        if (!package) {
            ...
            //mongoose.Types.ObjectId(req.body.claim.userId)
            var insId = mongoose.Types.ObjectId(req.body.package.insProviderId);
            var categoryId = mongoose.Types.ObjectId(req.body.package.insCategoryId);
            var insName;
            var categoryName;
            ...
            package.diseasesCovered = req.body.package.diseasesCovered,
                package.diseasesUnCovered = req.body.package.diseasesUnCovered,
                console.log(package);
